...
function CreateCourses() {
  ...
  const handleCreateCourse = async() => {
    setLoading(true);
    try {
      const result = await axios.post(serverUrl + "/api/courses/create", {title, category}, {withCredentials: true})
        ...
        <FaArrowLeftLong className='top-[8%] absolute left-[5%] w-[22px] h-[22px] cursor-pointer' onClick={() => navigate("/courses")}/>
          ...
          <form className='space-y-4'onSubmit={(e) =>e.preventDefault()}>
            ...
            <div>
              ...
              <select  id="category" className='w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[black]' onChange={(e) => setCategory(e.target.value)} value={category}>
            ...
            <button className='w-full bg-[black] text-white py-2 px-4 rounded-md  active:bg-[#3a3a3a] transition'disabled={loading} onClick={(e) => handleCreateCourse(e)}>{ loading ? <ClipLoader size={30} color='white'/> :"Create"}</button>
          </form>
      </div>
